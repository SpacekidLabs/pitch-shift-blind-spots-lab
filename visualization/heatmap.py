...
def _value_to_color(value: float, min_value: float, max_value: float) -> tuple[int, int, int]:
    ...
    stops = [
        ...
        (0.35, np.array([27, 109, 146])),
        (0.7, np.array([69, 170, 108])),
        (1.0, np.array([250, 214, 74])),
    ]
...
def _draw_panel(draw: ImageDraw.ImageDraw, x0: int, y0: int, width: int, height: int, title: str, frame: pd.DataFrame, value_col: str, row_labels: list[str]) -> None:
    ...
    draw.rectangle([x0, y0, x0 + panel_w, y0 + panel_h], outline=(40, 40, 40), width=1)
...
def save_stress_heatmap(results: pd.DataFrame, output_path: str | Path) -> None:
    output_path = Path(output_path)
    row_order = list(dict.fromkeys(results["signal_label"].tolist()))
    ordered = results.copy()
    ordered["signal_label"] = pd.Categorical(ordered["signal_label"], categories=row_order, ordered=True)
    ordered = ordered.sort_values(["signal_label", "shift_semitones"]).reset_index(drop=True)
    ...
    for column in ["rms_error", "spectral_distance", "spectral_centroid_difference"]:
        ...
        if max_v > min_v:
            normalized[column] = (values - min_v) / (max_v - min_v)
        else:
    ...
    panel_h = top_margin + len(row_order) * cell_h + 20
    margin = 24
    canvas_w = 2 * panel_w + 3 * margin
    ...
    positions = [
        (margin, margin),
        (margin * 2 + panel_w, margin),
        (margin, margin * 2 + panel_h),
        (margin * 2 + panel_w, margin * 2 + panel_h),
    ]
    for (title, column), (x0, y0) in zip(panel_titles, positions):
        _draw_panel(draw, x0, y0, panel_w, panel_h, title, normalized, column, row_order)
